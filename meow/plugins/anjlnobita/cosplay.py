...
async def waifu(event):
    ...
    await event.reply(file=r["url"], caption="Cosplay image")

@meow.on(events.NewMessage(pattern="^/lewd"))
async def waifu(event):
    r = requests.get("https://waifu-api.vercel.app/items/1").json()
    await event.reply(file=r["url"], caption="Lewd image")

__mod_name__ = "ᴄᴏsᴘʟᴀʏ"
__help__ = """ Just a weeb type module to get anime cosplay and lewd pictures - /cosplay - /lewd """